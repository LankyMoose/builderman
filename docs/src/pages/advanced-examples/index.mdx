...
const e2e = pipeline([smokeTests, fullTests]).toTask({
  ...
  maxConcurrency: (cmd) => cmd === "test" && !!process.env.CI ? 1 : Infinity,
})
...
Use `maxConcurrency` on `toTask()` to limit parallelism within a group of tasks, independently of the outer pipeline. This accepts either a fixed number or a function of the command name:

```ts
const e2e = pipeline([smokeTests, fullTests]).toTask({
  name: "e2e",
  // Run sequentially in CI for "test", unlimited for all other commands
  maxConcurrency: (cmd) => cmd === "test" && !!process.env.CI ? 1 : Infinity,
})
```

Tasks in the outer pipeline that the inner tasks depend on are excluded automatically — they won't be re-run inside the group.
