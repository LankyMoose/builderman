...
`maxConcurrency` can be a number, or a **function of the command name** so the limit varies per mode:

```ts
const e2eSuites = pipeline([a, b, c]).toTask({
  name: "e2e",
  // Sequential in CI for "test", unlimited otherwise
  maxConcurrency: (command) => command === "test" && !!process.env.CI ? 1 : Infinity,
})
